const icons = {
  ...
  cart: '<svg viewBox="0 0 24 24"><circle cx="9" cy="21" r="1"/><circle cx="20" cy="21" r="1"/><path d="M1 1h4l2.7 12.4a2 2 0 0 0 2 1.6h9.7a2 2 0 0 0 2-1.6L23 6H6"/></svg>',
  calendar: '<svg viewBox="0 0 24 24"><rect x="3" y="4" width="18" height="18" rx="2"/><path d="M16 2v4"/><path d="M8 2v4"/><path d="M3 10h18"/></svg>',
  comment: '<svg viewBox="0 0 24 24"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2v10Z"/></svg>',
  flag: '<svg viewBox="0 0 24 24"><path d="M4 15s1-1 4-1 5 2 8 2 4-1 4-1V3s-1 1-4 1-5-2-8-2-4 1-4 1z"/><path d="M4 22v-7"/></svg>',
  alert: '<svg viewBox="0 0 24 24"><path d="m21.73 18-8-14a2 2 0 0 0-3.48 0l-8 14A2 2 0 0 0 4 21h16a2 2 0 0 0 1.73-3Z"/><path d="M12 9v4"/><path d="M12 17h.01"/></svg>',
  check: '<svg viewBox="0 0 24 24"><path d="M20 6 9 17l-5-5"/></svg>',
  arrowRight: '<svg viewBox="0 0 24 24"><path d="M5 12h14"/><path d="m13 6 6 6-6 6"/></svg>',
  arrowLeft: '<svg viewBox="0 0 24 24"><path d="M19 12H5"/><path d="m11 6-6 6 6 6"/></svg>',
  arrowsBoth: '<svg viewBox="0 0 24 24"><path d="M8 7 4 12l4 5"/><path d="M16 7l4 5-4 5"/></svg>'
};
        ...
        .map(({ profile, index }) => {
          ...
          return `
            <tr class="dna-contact-row${active}" data-dna-index="${index}" tabindex="0" role="button" aria-pressed="${index === selectedDnaIndex}">
              <td class="grey-state"><span class="role-badge ${profile.roleClass}">${escapeHtml(profile.role)}</span></td>
              <td>
              ...
              <td class="grey-state">${renderInfluence(profile.influence)}</td>
              <td>
              ...
              <td class="grey-state"><span class="dna-score">${profile.interestScore}<small>/100</small></span></td>
              <td class="grey-state">${renderEngagementHealthPill(profile.engagementHealth)}</td>
              <td>
  ...
  container.innerHTML = `
    <article class="panel dna-card dna-profile-card">
      ...
      <div class="dna-card-body">
        ...
        <div class="dna-tag-cluster dna-tag-cluster--footer grey-state">
    ...
    <article class="panel dna-card dna-persona-card">
      ...
      <div class="dna-card-body">
        <div class="dna-persona-metrics grey-state">
        ...
        <div class="dna-tag-cluster grey-state">
        ...
        <div class="dna-persona-experience-block grey-state">
    ...
    <article class="panel dna-card dna-ownership-card grey-state">
...
function syncMatrix() {
  ...
  const delayed = document.querySelector("#systemTable .ops-sync-state.delayed");
  if (delayed) {
    delayed.textContent = "Synced";
    delayed.classList.remove("delayed");
    delayed.classList.add("synced");
  }
  const legacyWarning = document.querySelector("#systemTable .pill.warning");
  if (legacyWarning) {
    legacyWarning.textContent = "Connected";
    legacyWarning.classList.remove("warning");
    legacyWarning.classList.add("success");
  }
...
const ADOPTION_CHART_DATA = {
  breadth: {
    go: [44, 52, 48, 58, 54, 62, 52, 66, 74, 58, 48, 78],
    bts: [38, 42, 48, 54, 60, 64, 42, 48, 54, 60, 64, 68],
    exchange: [46, 56, 62, 38, 54, 44, 56, 62, 38, 54, 44, 62],
    analytic: [42, 50, 60, 48, 52, 34, 60, 48, 52, 34, 46, 54],
    servicenow: [32, 36, 28, 32, 48, 40, 36, 28, 32, 48, 40, 48],
    security: [34, 38, 50, 42, 28, 45, 38, 50, 42, 28, 34, 45],
  },
  depth: {
    go: [40, 48, 62, 56, 72, 58, 48, 62, 56, 72, 58, 68],
    bts: [44, 52, 44, 58, 46, 54, 52, 44, 58, 46, 54, 58],
    exchange: [36, 44, 52, 40, 54, 48, 44, 52, 40, 54, 48, 54],
    analytic: [28, 34, 42, 30, 38, 28, 34, 42, 30, 38, 28, 42],
    servicenow: [24, 28, 36, 24, 32, 20, 28, 36, 24, 32, 20, 38],
    security: [20, 32, 24, 28, 18, 26, 32, 24, 28, 18, 26, 32],
  },
};

function renderAdoptionChart(moduleEl, product) {
  const chartType = moduleEl.dataset.adoptionChart;
  const chartEl = moduleEl.querySelector("[data-adoption-bars]");
  const values = ADOPTION_CHART_DATA[chartType]?.[product];
  if (!chartEl || !values) return;

  chartEl.innerHTML = values
    .map(
      (value, index) =>
        `<div class="behavior-monthly-bar" style="--h: ${value}%"><span>${value}%</span><b>${ADOPTION_CHART_MONTHS[index]}</b></div>`
    )
    .join("");
}

function switchAdoptionProduct(button) {
  const moduleEl = button.closest("[data-adoption-chart]");
  if (!moduleEl) return;

  const product = button.dataset.adoptionProduct;
  moduleEl.querySelectorAll("[data-adoption-product]").forEach((item) => {
    const isActive = item === button;
    item.classList.toggle("active", isActive);
    item.setAttribute("aria-selected", isActive ? "true" : "false");
  });
  renderAdoptionChart(moduleEl, product);
}

function initAdoptionCharts() {
  document.querySelectorAll("[data-adoption-chart]").forEach((moduleEl) => {
    const activeButton =
      moduleEl.querySelector("[data-adoption-product].active") ||
      moduleEl.querySelector("[data-adoption-product]");
    if (activeButton) renderAdoptionChart(moduleEl, activeButton.dataset.adoptionProduct);
  });
}

const HEALTH_SCORE_HISTORY = {
  hour: {
    rangeLabel: "(Last 24 Hours)",
    bars: [
      88, 92, 86, 94, 78, 90, 87, 93, 85, 91, 89, 76, 88, 92, 86, 94, 90, 87, 93, 89, 91, 88,
      92, 90,
    ],
    riskAt: [4, 11],
    axis: ["16:00", "20:00", "00:00", "04:00", "08:00", "12:00"],
  },
  day: {
    rangeLabel: "(Last 30 Days)",
    bars: [
      88, 92, 86, 94, 78, 90, 87, 93, 85, 91, 89, 76, 88, 92, 86, 94, 90, 82, 87, 93, 89, 91,
      86, 94, 88, 92, 90, 95, 89, 93,
    ],
    riskAt: [4, 11, 17],
    axis: ["4/19", "4/24", "4/29", "5/4", "5/9", "5/14", "5/18"],
  },
  month: {
    rangeLabel: "(Last 12 Months)",
    bars: [88, 91, 86, 93, 78, 90, 87, 93, 85, 91, 89, 95],
    riskAt: [4],
    axis: ["7/24", "9/24", "11/24", "1/25", "3/25", "5/25"],
  },
};

function renderHealthScoreHistory(root, granularity) {
  const config = HEALTH_SCORE_HISTORY[granularity];
  if (!config || !root) return;

  const barsEl = root.querySelector("[data-health-history-bars]");
  const axisEl = root.querySelector("[data-health-history-axis]");
  const rangeLabel = root.querySelector("[data-health-history-range-label]");
  if (!barsEl || !axisEl) return;

  const riskSet = new Set(config.riskAt || []);

  barsEl.dataset.barCount = String(config.bars.length);
  barsEl.style.setProperty("--health-bar-count", String(config.bars.length));
  barsEl.innerHTML = config.bars
    .map((height, index) => {
      const tone = riskSet.has(index) || height < 85 ? "risk" : "health";
      return `<div class="health-history-bar health-history-bar--${tone}" style="--h: ${height}%"><span></span></div>`;
    })
    .join("");

  axisEl.innerHTML = config.axis.map((label) => `<span>${label}</span>`).join("");
  if (rangeLabel) rangeLabel.textContent = config.rangeLabel;
}

function switchHealthScoreGranularity(button) {
  const root = button.closest("[data-health-score-history]");
  if (!root) return;

  const granularity = button.dataset.healthHistoryGranularity;
  root.querySelectorAll("[data-health-history-granularity]").forEach((item) => {
    const isActive = item === button;
    item.classList.toggle("active", isActive);
    item.setAttribute("aria-selected", isActive ? "true" : "false");
  });
  renderHealthScoreHistory(root, granularity);
}

function initHealthScoreHistory() {
  document.querySelectorAll("[data-health-score-history]").forEach((root) => {
    const activeButton =
      root.querySelector("[data-health-history-granularity].active") ||
      root.querySelector('[data-health-history-granularity="day"]');
    if (activeButton) {
      renderHealthScoreHistory(root, activeButton.dataset.healthHistoryGranularity);
    }
  });
}
...
function wireEvents() {
  ...
  document.querySelectorAll("[data-adoption-product]").forEach((button) => {
    button.addEventListener("click", () => switchAdoptionProduct(button));
  });

  document.querySelectorAll("[data-health-history-granularity]").forEach((button) => {
    button.addEventListener("click", () => switchHealthScoreGranularity(button));
  });
  ...
  const operationsSyncRefresh = document.getElementById("operationsSyncRefresh");
  if (operationsSyncRefresh) operationsSyncRefresh.addEventListener("click", syncMatrix);
  const operationsFilterClear = document.getElementById("operationsFilterClear");
  if (operationsFilterClear) {
    operationsFilterClear.addEventListener("click", () => {
      document.querySelectorAll(".operations-filter select").forEach((select) => {
